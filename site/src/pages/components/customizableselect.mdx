...
This example uses the customizable `<select>` element with custom CSS to target it which renders the option differently in the listbox vs in the button. The emojis have aria-hidden on them to prevent the name of the emoji being announced twice in the popup.
...
<select>
  ...
  <option>
    <span aria-hidden=true>❤️</span> <span class=description>heart</span>
  </option>
  <option>
    <span aria-hidden=true>🙂</span> <span class=description>smile</span>
  </option>
</select>
<style>
